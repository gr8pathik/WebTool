...
class Auth {
    ...
    function check_password($password) 
	{
        ...
        $query = $this->CI->db->where($this->_table['users'], array('email' => $this->_username), 1)->row();
	...
	{
        $this->CI->db->where('(email = "'.$email.'" OR username = "'.$email.'")');
        ...
        if ($query->num_rows() === 1) 
		{
            ...
            $data['logged_in'] = TRUE;
            $data['sess_expire_on_close'] = TRUE;
            unset($data['password']);
		...
		else 
		{
            ...
            if ($error_view != NULL) 
			{
                $this->CI->template->build($error_view);
